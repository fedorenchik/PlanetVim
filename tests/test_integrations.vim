...
try
  ...
  if ! has('win32') && executable('bash')
    ...
    call assert_equal('value with spaces', planet#project#Context().env_snapshot.PLANETVIM_SDK_FIXTURE)
    call assert_equal(s:previous, getenv('PLANETVIM_SDK_FIXTURE'), 'SDK activation cannot change other projects')
    ...
    call assert_equal('success', s:Wait(planet#integrations#Environment('fixture', s:setup, [], #{hidden:v:true})).status)
    call assert_false(has_key(planet#project#Context().env_snapshot, 'PLANETVIM_SDK_REMOVED'))
